...
if (main.cartShipping.payment_sessions.length < 1) {
  await client.carts
...
await client.carts
  .setPaymentSession(main.cartShipping.id, { provider_id: "manual" })
  .then(({ cart }) => {
    main.setShipCart(cart);
  });
console.log(main.cartShipping.payment_session.data);
